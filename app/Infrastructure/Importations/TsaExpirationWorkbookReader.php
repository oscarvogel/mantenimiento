...
final class TsaExpirationWorkbookReader implements SpreadsheetReader
{
    ...
    {
        $header = $this->findRow($matrix, static fn (array $normalized): bool =>
            in_array('placa', $normalized, true) && in_array('vtv', $normalized, true)
        );
        if ($header === null) {
            return [];
        }
            ...
            foreach ($groups as $group) {
                ...
                foreach ($group['fields'] as $field => $column) {
                    ...
                    $rows[] = [
                        ...
                        'observaciones' => 'Fuente TSA vencimientos; sucursal sugerida ' . $group['branch'] . '; fila ' . ($header['row'] + $sourceRow + 2) . '.',
                    ];
                }
            }
        }
    ...
    {
        $header = $this->findRow($matrix, static fn (array $normalized): bool => in_array('chofer', $normalized, true));
        if ($header === null) {
            return [];
        }
        ...
        foreach ($header['values'] as $column => $name) {
            ...
            $next = array_search('chofer', array_slice($header['values'], $column + 1), true);
            $end = $next === false ? count($header['values']) : $column + 1 + (int) $next;
        ...
        foreach (array_slice($matrix, $header['row'] + 1) as $sourceRow => $values) {
            foreach ($driverColumns as $column => $label) {
                $nameColumn = $this->driverNameColumn($header['values'], $column);
                $driver = $this->text($values[$nameColumn] ?? null);
                $date = $this->date($values[$column] ?? null);
                if ($driver === '' || $date === null) {
                    continue;
                }
                ...
                $rows[] = [
                    ...
                    'equipo_codigo' => null,
                    'empleado_nombre' => $driver,
                    'tipo_vencimiento' => 'LICENCIA_CHOFER',
                    'fecha_vencimiento' => $date,
                    'fecha_emision' => null,
                    'numero_documento' => null,
                    'observaciones' => 'Fuente TSA choferes (' . $label . '), fila ' . ($header['row'] + $sourceRow + 2) . '.',
                ];
            }
        }
    ...
    private function canonicalHeader(array $matrix): ?array
    {
        foreach ($matrix as $row => $values) {
            ...
            if (in_array('sujeto_tipo', $headers, true) && in_array('tipo_vencimiento', $headers, true)) {
    ...
    private function date(mixed $value): ?string
    {
        ...
        foreach (['!Y-m-d', '!d/m/Y', '!d/n/Y', '!j/m/Y', '!j/n/Y'] as $format) {
    ...
    {
        if (strtolower(pathinfo($path, PATHINFO_EXTENSION)) !== 'xlsx' || ! is_file($path)) {
            throw new DomainException('El archivo de vencimientos debe ser un XLSX válido.');
        }
        if (! class_exists(IOFactory::class) || ! class_exists(\ZipArchive::class)) {
            throw new DomainException('No se puede leer XLSX: falta habilitar la extensión PHP zip.');
        }
