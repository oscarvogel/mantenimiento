...
final class CodeIgniterExpirationImportGateway implements ExpirationImportGateway
{
    ...
    public function isDuplicate(ExpirationImportData $data): bool
    {
        ...
        $data->subjectType === ExpirationSubjectType::EQUIPMENT
            ? $builder->where('equipo_id', $data->subjectId)
            : $builder->where('empleado_id', $data->subjectId);
    ...
    public function import(ExpirationImportData $data): int
    {
        ...
        if ($typeId === null) {
            ...
            $this->database->table('tipos_vencimiento')->insert([
                ...
                'aplica_a' => $data->subjectType === ExpirationSubjectType::EQUIPMENT ? 'EQUIPO' : 'EMPLEADO',
        ...
        $this->database->table('vencimientos')->insert([
            ...
            'sujeto_tipo' => $data->subjectType->value,
            'equipo_id' => $data->subjectType === ExpirationSubjectType::EQUIPMENT ? $data->subjectId : null,
            'empleado_id' => $data->subjectType === ExpirationSubjectType::EMPLOYEE ? $data->subjectId : null,
    ...
    private function assertSubjectExists(ExpirationImportData $data): void
    {
        if ($data->subjectType === ExpirationSubjectType::EQUIPMENT) {
            ...
            if ($data->branchId !== null) {
                $builder->where('sucursal_id', $data->branchId);
            }
        ...
        if ($this->database->table('empleados')
            ->where('empresa_id', $data->companyId)
            ->where('id', $data->subjectId)
            ->where('activo', 1)
            ->where('deleted_at', null)
            ->countAllResults() === 0) {
            throw new DomainException('El empleado ya no existe o está dado de baja.');
        }
    }
    ...
    private function findTypeId(int $companyId, string $name): ?int
    {
        ...
        foreach ($this->database->table('tipos_vencimiento')
            ->select('id, nombre')
    ...
    private function canonicalTypeName(string $name): string
    {
        return match (mb_strtoupper(trim($name))) {
            'POLIZA', 'PÓLIZA', 'SEGURO', 'SEGURO_AUTOMOTOR' => 'Póliza',
            'VTV', 'ITV' => 'VTV / ITV',
            'SENASA' => 'SENASA',
            'CRVL' => 'CRVL',
            'LICENCIA_CHOFER', 'LICENCIA', 'REGISTRO' => 'Licencia de conducir',
            default => trim($name),
        };
    }
}
