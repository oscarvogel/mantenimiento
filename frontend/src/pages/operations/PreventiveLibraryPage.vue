...
const filteredItems = computed(() => {
  ...
  return items.value.filter((item) => {
    if (selectedTemplateId.value !== null && item.templateId !== selectedTemplateId.value) {
      return false
    }
...
function persistEditHint(item) {
  ...
  try {
    window.localStorage.setItem(EDIT_HINT_KEY(item.templateId, item.id), '1')
  } catch {
    /* ignore */
  }
}
...
function readStoredHints() {
  ...
  try {
    for (let i = 0; i < window.localStorage.length; i += 1) {
      ...
      if (key.startsWith('biblioteca:edit:lastItem:')) {
        ...
        if (selectedTemplateId.value === null || templateId === selectedTemplateId.value) {
          expand.add(itemId)
        }
      }
      if (key.startsWith('biblioteca:edit:lastTask:')) {
  ...
  if (expand.size > 0) {
    expandedItems.value = new Set([...expandedItems.value, ...expand])
  }
}
...
<template>
  <div>
    <PageHeading
      ...
      description="Servicios y tareas del catálogo compartido. Elegí una plantilla para ver sus servicios."
      ...
      <div class="flex min-w-0 flex-1 flex-col gap-1">
        ...
        <div class="relative">
          <select
            ...
            class="block min-h-11 w-full appearance-none rounded-lg border border-border bg-surface-raised pl-3 pr-9 text-sm font-semibold text-ink shadow-sm focus:border-border-focus focus:outline-none focus:ring-2 focus:ring-primary/15 disabled:cursor-not-allowed disabled:bg-surface-muted"
        ...
        <p v-if="selectedTemplate" class="font-mono text-xs text-ink-muted">
          {{ selectedTemplate.code }} - {{ selectedTemplate.equipmentType }} - {{ selectedTemplate.itemCount }} ítems
        </p>
      </div>
      <div class="flex min-w-0 flex-1 flex-col gap-1">
        ...
        <label for="library-search" class="relative block">
          ...
          <input
            ...
            name="q"
            type="search"
            placeholder="Servicio, código o tarea"
            :class="fieldClass + ' pl-9'"
          />
        </label>
      </div>
      <a :href="data.routes.downloadTemplate" :class="secondaryButton">
        <ArrowDownTrayIcon class="mr-2 size-4" aria-hidden="true" />Descargar plantilla
      </a>
    </div>
    ...
    <div class="mb-4 flex flex-wrap items-center gap-2 text-xs">
      ...
      <span class="inline-flex items-center gap-1.5 rounded-full bg-surface-muted px-2.5 py-1 font-semibold text-ink-muted">
        Servicios<strong class="text-ink">{{ serviceCountInTemplate }}</strong>
      </span>
      <span class="inline-flex items-center gap-1.5 rounded-full bg-surface-muted px-2.5 py-1 font-semibold text-ink-muted">
      ...
      <span
        v-if="searchActive"
        class="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 font-semibold text-primary"
      >Filtrado</span>
    </div>

    <EmptyState
      v-if="items.length === 0"
      title="Todavía no hay biblioteca importada"
      description="Importá la biblioteca preventiva desde Excel para crear la primera plantilla y sus servicios."
    />
    <EmptyState
      v-else-if="filteredItems.length === 0"
      :title="searchActive ? 'Sin resultados para la búsqueda' : 'Esta plantilla no tiene servicios'"
      :description="searchActive
        ? 'Probá con otro servicio, código o palabra clave dentro de la plantilla seleccionada.'
        : 'Elegí otra plantilla o importá más servicios para esta.'"
    />
      ...
      >
        <div class="flex flex-wrap items-center gap-3 px-4 py-3">
          <button
            ...
            :aria-controls="'library-item-' + item.id"
        ...
        <div
          ...
          class="border-t border-border-subtle bg-surface-subtle/60 px-4 py-3"
        >
          <p v-if="!item.tasks || item.tasks.length === 0" class="text-xs text-ink-muted">
            Este servicio aún no tiene tareas definidas en el catálogo compartido.
          </p>
          <ul v-else class="divide-y divide-border-subtle overflow-hidden rounded-lg border border-border-subtle bg-white">
            <li
              v-for="task in item.tasks"
              :key="task.id"
              class="flex flex-wrap items-center gap-3 px-3 py-2 sm:px-4"
            >
