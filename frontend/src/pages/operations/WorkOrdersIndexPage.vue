...
import { ArrowPathIcon, MagnifyingGlassIcon, PrinterIcon } from '@heroicons/vue/24/outline'
...
<template>
  <div>
    ...
    <div v-else class="space-y-3">
      <article v-for="order in data.orders" :key="order.id" class="rounded-xl border border-border bg-surface-raised p-4 sm:p-5">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div class="flex flex-wrap gap-2 lg:justify-end">
            ...
            <a v-if="order.status === 'EN_PROCESO' && data.can.closeOrder" :href="`${data.routes.maintenance}#orden-${order.id}`" :class="secondaryButton">Cerrar</a>
          </div>
        </div>
      </article>
    </div>
