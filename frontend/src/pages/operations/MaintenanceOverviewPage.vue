...
const stateFor = (equipment) => {
  if (!planForms[equipment.id]) {
    ...
    planForms[equipment.id] = {
      serviceId,
      intervalo_km: '', anticipacion_km: '', intervalo_horas: '', anticipacion_horas: '', intervalo_dias: '', anticipacion_dias: '', prioridad: 'MEDIA', observaciones: '',
    }
...
const openCloseModal = (order) => { closeStateFor(order); activeCloseOrder.value = order }
const closeCloseModal = () => { activeCloseOrder.value = null }
const updateCloseForm = (value) => { if (activeCloseOrder.value) closeForms[activeCloseOrder.value.id] = value }
...
<template>
  <div>
    <PageHeading eyebrow="Centro operativo" title="Qué requiere atención hoy" description="Priorizá vencimientos, avisos y órdenes antes de avanzar con tareas administrativas.">
      <template #actions><a :href="data.routes.equipmentIndex" :class="secondaryButton">Ver todos los equipos<ArrowRightIcon class="ml-2 size-4" aria-hidden="true" /></a></template>
    </PageHeading>
    ...
    <div class="mb-6 grid gap-6 xl:grid-cols-[minmax(0,1.65fr)_minmax(18rem,0.8fr)]">
      ...
      <PanelCard title="Órdenes de trabajo" :count="data.pagination.orders.total">
        ...
        <div v-else class="space-y-4"><article v-for="order in data.orders" :key="order.id" class="rounded-xl border border-border p-4"><div class="flex items-start justify-between gap-3"><div class="flex items-center gap-3"><EquipmentThumbnail :url="order.photoUrl" :code="order.equipmentCode" /><strong class="text-ink">{{ order.number }} · {{ order.equipmentCode }}</strong></div><StatusBadge :status="order.status" /></div><p class="mt-2 text-sm text-ink-muted">{{ order.serviceName || 'Servicio preventivo' }} · Responsable: {{ order.ownerName || 'Sin asignar' }}</p><ul v-if="order.tasks.length" class="mt-3 space-y-1 text-sm text-ink"><li v-for="task in order.tasks" :key="task.id">{{ task.description }} <span class="text-ink-muted">({{ task.status }})</span></li></ul><div class="mt-4 flex flex-wrap gap-2"><a :href="printOrderUrl(order)" target="_blank" rel="noopener" :class="secondaryButton">Imprimir OT</a><form v-if="order.status === 'EMITIDA' && data.can.editOrder" method="post" :action="order.startUrl"><CsrfInput :csrf="data.csrf" /><button type="submit" :class="secondaryButton">Iniciar orden</button></form><button v-else-if="order.status === 'EN_PROCESO' && data.can.closeOrder" type="button" :class="secondaryButton" aria-haspopup="dialog" @click="openCloseModal(order)"><WrenchScrewdriverIcon class="mr-2 size-4" aria-hidden="true" />Cerrar orden</button></div></article></div>
    ...
    <div v-if="correctiveModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="presentation" @click.self="closeCorrectiveModal">
      <section class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-surface-raised shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="corrective-modal-title">
        <div class="flex items-start justify-between gap-4 border-b border-border-subtle px-5 py-4 sm:px-6"><div><p class="text-xs font-bold uppercase tracking-wide text-primary">Trabajo correctivo</p><h2 id="corrective-modal-title" class="mt-1 text-xl font-bold text-ink">Registrar trabajo realizado</h2><p class="mt-1 text-sm text-ink-muted">El trabajo ya fue realizado. Se registra directamente como finalizado, sin abrir ni iniciar una OT.</p></div><button type="button" class="rounded-lg p-2 text-ink-muted hover:bg-surface-muted hover:text-ink" aria-label="Cerrar modal" @click="closeCorrectiveModal"><XMarkIcon class="size-5" aria-hidden="true" /></button></div>
        <form method="post" enctype="multipart/form-data" :action="correctiveOrderUrl" class="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
          <CsrfInput :csrf="data.csrf" />
          <input type="hidden" name="equipo_id" :value="correctiveEquipmentId" />
          <input type="hidden" name="volver_equipo" :value="correctiveEquipmentLocked ? '1' : '0'" />
          <FormField label="Equipo *" for-id="corrective-equipment-search" class="sm:col-span-2">
            <div v-if="correctiveEquipmentLocked && selectedCorrectiveEquipment" class="rounded-xl border border-success/20 bg-success-subtle px-4 py-3"><p class="text-xs font-semibold uppercase tracking-wide text-success-strong">Equipo de esta ficha</p><p class="mt-1 font-bold text-ink">{{ selectedCorrectiveEquipment.code }}<span v-if="selectedCorrectiveEquipment.plate"> · {{ selectedCorrectiveEquipment.plate }}</span></p></div>
            <template v-else><div class="relative"><MagnifyingGlassIcon class="pointer-events-none absolute left-3 top-3 size-5 text-ink-subtle" aria-hidden="true" /><input id="corrective-equipment-search" v-model="correctiveSearch" type="search" autocomplete="off" required placeholder="Buscar por código o patente" :class="`${fieldClass} pl-10`" @input="correctiveEquipmentId = ''" /></div><div v-if="!selectedCorrectiveEquipment" class="mt-2 max-h-44 overflow-y-auto rounded-xl border border-border bg-surface"><button v-for="equipment in filteredCorrectiveEquipments" :key="equipment.id" type="button" class="flex w-full items-center justify-between gap-3 border-b border-border-subtle px-3 py-2 text-left last:border-0 hover:bg-surface-muted" @click="selectCorrectiveEquipment(equipment)"><span><strong class="text-sm text-ink">{{ equipment.code }}</strong><span v-if="equipment.plate" class="ml-2 text-xs text-ink-muted">{{ equipment.plate }}</span></span><span class="text-xs text-ink-subtle">{{ equipment.typeName }}</span></button><p v-if="filteredCorrectiveEquipments.length === 0" class="px-3 py-3 text-sm text-ink-muted">No se encontraron equipos.</p></div><p v-else class="mt-2 rounded-lg bg-success-subtle px-3 py-2 text-sm font-semibold text-success-strong">Seleccionado: {{ selectedCorrectiveEquipment.code }}<span v-if="selectedCorrectiveEquipment.plate"> · {{ selectedCorrectiveEquipment.plate }}</span></p></template>
          </FormField>
          <FormField label="Fecha del trabajo *" for-id="corrective-date"><input id="corrective-date" type="date" name="fecha_servicio" :value="today()" required :class="fieldClass" /></FormField>
          <FormField label="Prioridad" for-id="corrective-priority"><select id="corrective-priority" name="prioridad" :class="fieldClass"><option value="MEDIA" selected>Normal</option><option value="ALTA">Alta</option><option value="CRITICA">Urgente</option></select></FormField>
          <FormField label="Problema / motivo *" for-id="corrective-problem" hint="Qué originó el trabajo." class="sm:col-span-2"><textarea id="corrective-problem" name="problema_reportado" rows="2" minlength="5" maxlength="3000" required placeholder="Ej.: pérdida de aceite hidráulico" :class="fieldClass"></textarea></FormField>
          <FormField label="Trabajo realizado *" for-id="corrective-work" hint="Qué se hizo realmente." class="sm:col-span-2"><textarea id="corrective-work" name="trabajo_realizado_correctivo" rows="3" minlength="5" maxlength="5000" required placeholder="Ej.: reemplazo de manguera de retorno y control de pérdidas" :class="fieldClass"></textarea></FormField>
          ...
          <template v-if="selectedCorrectiveEquipment"><FormField v-if="selectedCorrectiveEquipment.controlsKm" label="Kilometraje al realizar el trabajo" for-id="corrective-km"><input id="corrective-km" type="number" min="0" name="km_salida" :value="selectedCorrectiveEquipment.currentKm ?? ''" :class="fieldClass" /></FormField><FormField v-if="selectedCorrectiveEquipment.controlsHours" label="Horómetro al realizar el trabajo" for-id="corrective-hours"><input id="corrective-hours" type="number" min="0" step="0.1" name="horas_salida" :value="selectedCorrectiveEquipment.currentHours ?? ''" :class="fieldClass" /></FormField></template>
          <div class="sm:col-span-2 grid gap-3 sm:grid-cols-3"><FormField label="Mano de obra" for-id="corrective-labor"><input id="corrective-labor" type="number" min="0" step="0.01" name="costo_mano_obra" value="0" :class="fieldClass" /></FormField><FormField label="Repuestos" for-id="corrective-parts"><input id="corrective-parts" type="number" min="0" step="0.01" name="costo_repuestos" value="0" :class="fieldClass" /></FormField><FormField label="Otros costos" for-id="corrective-other"><input id="corrective-other" type="number" min="0" step="0.01" name="otros_costos" value="0" :class="fieldClass" /></FormField></div>
          <FormField label="Evidencia opcional" for-id="corrective-evidence" hint="PDF, JPG, PNG o WEBP. Puede ser el remito, una foto o un comprobante." class="sm:col-span-2"><input id="corrective-evidence" type="file" name="evidencia" accept=".pdf,.jpg,.jpeg,.png,.webp,application/pdf,image/jpeg,image/png,image/webp" :class="fieldClass" /></FormField>
          <div class="rounded-lg bg-info-subtle px-4 py-3 text-sm text-info-strong sm:col-span-2">Al guardar, la correctiva quedará <strong>FINALIZADA</strong>. Si informás km u horas, también se registrará la lectura y se recalculará el mantenimiento preventivo.</div>
          <div class="flex flex-col-reverse gap-2 border-t border-border-subtle pt-4 sm:col-span-2 sm:flex-row sm:justify-end"><button type="button" :class="secondaryButton" @click="closeCorrectiveModal">Cancelar</button><button type="submit" :disabled="!correctiveEquipmentId" :class="primaryButton"><PlusIcon class="mr-2 size-4" aria-hidden="true" />Registrar trabajo realizado</button></div>
        </form>
      </section>
    </div>

    <WorkOrderClosureModal v-if="activeCloseOrder" :order="activeCloseOrder" :csrf="data.csrf" :form-state="closeStateFor(activeCloseOrder)" @close="closeCloseModal" @update:form-state="updateCloseForm" />
  </div>
</template>
