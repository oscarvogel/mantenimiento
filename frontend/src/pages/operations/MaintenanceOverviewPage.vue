...
<template>
  <div>
    ...
    <PanelCard title="2. Vencimientos y avisos" class="mb-6">
      ...
      <div v-else class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><article v-for="plan in data.plans" :key="plan.id" class="rounded-xl border border-border p-4"><div class="flex items-start justify-between gap-3"><div class="flex items-center gap-2"><EquipmentThumbnail :url="plan.photoUrl" :code="plan.equipmentCode" size="sm" /><strong class="text-ink">{{ plan.equipmentCode }}</strong></div><StatusBadge :status="plan.computedState || 'SIN_DATOS'" /></div><p class="mt-2 text-sm text-ink">{{ plan.serviceName }}</p><p class="mt-2 text-xs text-ink-muted">Próximo: <span v-if="plan.nextKm !== null">{{ plan.nextKm }} km </span><span v-if="plan.nextHours !== null">{{ plan.nextHours }} h </span><span v-if="plan.nextDate !== null">{{ plan.nextDate }}</span></p></article></div>
