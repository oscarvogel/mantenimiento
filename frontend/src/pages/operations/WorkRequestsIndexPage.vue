...
import { animateFlip, captureFlipState, panelTransitionHooks } from '../../ui/gsapMotion.js'

const props = defineProps({ data: { type: Object, required: true } })
const reviewForms = reactive({})
const requestList = ref(null)
const reviewTransition = panelTransitionHooks({ duration: 0.2 })
...
const animateRequestLayout = async (mutate) => {
  const state = captureFlipState(requestList.value?.querySelectorAll('[data-request-card]'))
  mutate()
  await nextTick()
  animateFlip(state, requestList.value?.querySelectorAll('[data-request-card]'))
}
const openReview = (request, status) => animateRequestLayout(() => {
  formFor(request).status = status
  formFor(request).reason = ''
  formFor(request).groupId = ''
})
const closeReview = (request) => animateRequestLayout(() => { formFor(request).status = '' })
...
<template>
  <div>
    ...
    <section class="overflow-hidden rounded-xl border border-border bg-surface-raised" aria-labelledby="requests-title">
      ...
      <div v-else ref="requestList" class="divide-y divide-border-subtle">
        <article v-for="request in data.items" :key="request.id" v-motion="{ y: 8 }" data-request-card class="p-5 sm:p-6">
          ...
          <Transition
            @before-enter="reviewTransition.beforeEnter"
            @enter="reviewTransition.enter"
            @leave="reviewTransition.leave"
          >
