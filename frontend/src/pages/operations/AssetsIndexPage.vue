<script setup>
import { CalendarDaysIcon, ChevronDownIcon, FunnelIcon, PlusIcon, QrCodeIcon, WrenchScrewdriverIcon, XMarkIcon } from '@heroicons/vue/24/outline'
...
import { dangerButton, fieldClass, formatHours, formatKilometers, nowLocal, primaryButton, secondaryButton, today } from './helpers.js'
...
<template>
  <div>
    ...
    <PanelCard title="Listado de equipos" :count="data.equipment.total" flush class="mb-6">
      <form method="get" :action="data.routes.index" class="grid gap-4 border-b border-border-subtle p-5 sm:grid-cols-2 sm:p-6 2xl:grid-cols-[minmax(15rem,1.4fr)_repeat(4,minmax(9rem,1fr))]">
        ...
        <FormField label="Sucursal" for-id="asset-filter-branch"><select id="asset-filter-branch" name="sucursal_id" :class="fieldClass"><option value="">Todas</option><option v-for="branch in data.catalogs.branches" :key="branch.id" :value="branch.id" :selected="String(data.filters.branchId) === String(branch.id)">{{ branch.name }}</option></select></FormField>
        ...
        <div class="flex flex-wrap gap-2 sm:col-span-2 2xl:col-span-5"><button type="submit" :class="primaryButton"><FunnelIcon class="mr-2 size-5" aria-hidden="true" />Aplicar filtros</button><a :href="data.routes.index" :class="secondaryButton">Limpiar</a></div>
      </form>
      ...
      <template v-else>
        <div class="hidden overflow-x-auto lg:block">
          <table class="w-full min-w-[62rem] text-left text-sm">
            ...
            <tbody class="divide-y divide-border-subtle"><tr v-for="equipment in data.equipment.items" :key="equipment.id" class="hover:bg-brand-50/60"><td class="px-6 py-4"><div class="flex items-center gap-3"><EquipmentThumbnail :url="equipment.photoUrl" :code="equipment.code" /><div><strong class="text-ink">{{ equipment.code }}</strong><br><span class="text-xs text-ink-muted">{{ equipment.typeName }}<template v-if="equipment.plate"> · {{ equipment.plate }}</template></span></div></div></td><td class="px-6 py-4 text-ink">{{ equipment.brandName || 'Sin marca' }}<template v-if="equipment.modelName"> · {{ equipment.modelName }}</template><br><span class="text-xs text-ink-muted">{{ equipment.year || 'Año sin informar' }}</span></td><td class="px-6 py-4 text-ink-muted">{{ equipment.branchCode }} · {{ equipment.branchName }}</td><td class="px-6 py-4 text-ink"><span v-if="equipment.controlsKm">{{ usageLabel(formatKilometers, equipment.currentKm) }}</span><br v-if="equipment.controlsKm && equipment.controlsHours"><span v-if="equipment.controlsHours">{{ usageLabel(formatHours, equipment.currentHours) }}</span></td><td class="px-6 py-4"><StatusBadge :status="equipment.status" /></td><td class="whitespace-nowrap px-6 py-4"><div class="flex flex-nowrap justify-end gap-2"><a :href="equipment.detailUrl" :class="secondaryButton">Ficha</a><details class="ui-details-animated"><summary :class="`${secondaryButton} cursor-pointer list-none`">Más acciones <ChevronDownIcon class="ml-1.5 size-4" aria-hidden="true" /></summary><div class="mt-2 flex flex-wrap justify-end gap-2 rounded-lg border border-border-subtle bg-surface-subtle p-2"><a :href="equipment.qrUrl" target="_blank" rel="noopener" :class="secondaryButton"><QrCodeIcon class="mr-1.5 size-4" aria-hidden="true" />Ver QR</a><form v-if="equipment.regenerateQrUrl" method="post" :action="equipment.regenerateQrUrl" data-confirm data-confirm-title="¿Regenerar QR?" data-confirm-text="El QR anterior dejará de funcionar inmediatamente." data-confirm-button="Regenerar"><CsrfInput :csrf="data.csrf" /><button type="submit" :class="secondaryButton">Regenerar QR</button></form><a v-if="equipment.assignPlanUrl" :href="equipment.assignPlanUrl" :class="secondaryButton"><CalendarDaysIcon class="mr-1.5 size-4" aria-hidden="true" />Asignar plan</a></div></details></div></td></tr></tbody>
          </table>
        </div>
        <ul class="divide-y divide-border-subtle lg:hidden"><li v-for="equipment in data.equipment.items" :key="equipment.id" class="p-5"><div class="flex items-start justify-between gap-3"><div class="flex gap-3"><EquipmentThumbnail :url="equipment.photoUrl" :code="equipment.code" size="lg" /><div><p class="font-bold text-ink">{{ equipment.code }}</p><p class="mt-1 text-sm text-ink-muted">{{ equipment.typeName }} · {{ equipment.branchName }}</p></div></div><StatusBadge :status="equipment.status" /></div><dl class="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt class="text-xs text-ink-muted">Ficha</dt><dd class="mt-1 font-medium text-ink">{{ equipment.brandName || 'Sin marca' }} {{ equipment.modelName || '' }}</dd></div><div><dt class="text-xs text-ink-muted">Uso</dt><dd class="mt-1 font-medium text-ink"><template v-if="equipment.controlsKm">{{ usageLabel(formatKilometers, equipment.currentKm) }}</template><br v-if="equipment.controlsKm && equipment.controlsHours"><template v-if="equipment.controlsHours">{{ usageLabel(formatHours, equipment.currentHours) }}</template></dd></div></dl><div class="mt-4 flex flex-wrap gap-2"><a :href="equipment.detailUrl" :class="`${secondaryButton} flex-1`">Ver ficha</a><details class="ui-details-animated min-w-[8.5rem] flex-1"><summary :class="`${secondaryButton} w-full cursor-pointer list-none`">Más acciones <ChevronDownIcon class="ml-1.5 size-4" aria-hidden="true" /></summary><div class="mt-2 grid gap-2 rounded-lg border border-border-subtle bg-surface-subtle p-2"><a :href="equipment.qrUrl" target="_blank" rel="noopener" :class="`${secondaryButton} w-full`">Ver QR</a><form v-if="equipment.regenerateQrUrl" method="post" :action="equipment.regenerateQrUrl" data-confirm data-confirm-title="¿Regenerar QR?" data-confirm-text="El QR anterior dejará de funcionar inmediatamente." data-confirm-button="Regenerar"><CsrfInput :csrf="data.csrf" /><button type="submit" :class="`${secondaryButton} w-full`">Regenerar QR</button></form><a v-if="equipment.assignPlanUrl" :class="`${secondaryButton} w-full`" :href="equipment.assignPlanUrl">Asignar plan</a></div></details></div></li></ul>
      </template>
