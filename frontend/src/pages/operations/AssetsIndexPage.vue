...
<template>
  <div>
    ...
    <PanelCard v-if="data.canEdit" title="Alta directa de camión" class="mb-6">
      <p class="mb-5 text-sm text-ink-muted">Registrá la unidad y su ficha técnica. El servidor validará empresa, sucursal, tipo, marca y modelo.</p>
    ...
    <PanelCard title="Listado de equipos" :count="data.equipment.total" flush class="mb-6">
      ...
      <template v-else>
        ...
        <ul class="divide-y divide-border-subtle lg:hidden"><li v-for="equipment in data.equipment.items" :key="equipment.id" class="p-5"><div class="flex items-start justify-between gap-3"><div class="flex gap-3"><EquipmentThumbnail :url="equipment.photoUrl" :code="equipment.code" size="lg" /><div><p class="font-bold text-ink">{{ equipment.code }}</p><p class="mt-1 text-sm text-ink-muted">{{ equipment.typeName }} · {{ equipment.branchName }}</p></div></div><StatusBadge :status="equipment.status" /></div><dl class="mt-4 grid grid-cols-2 gap-3 text-sm"><div><dt class="text-xs text-ink-muted">Ficha</dt><dd class="mt-1 font-medium text-ink">{{ equipment.brandName || 'Sin marca' }} {{ equipment.modelName || '' }}</dd></div><div><dt class="text-xs text-ink-muted">Uso</dt><dd class="mt-1 font-medium text-ink">{{ equipment.currentKm === null ? '—' : `${equipment.currentKm} km` }}<br>{{ equipment.currentHours === null ? '—' : `${equipment.currentHours} h` }}</dd></div></dl><div class="mt-4 flex gap-2"><a :href="equipment.detailUrl" :class="`${secondaryButton} flex-1`">Ver ficha</a><a :href="equipment.qrUrl" target="_blank" rel="noopener" :class="secondaryButton" aria-label="Abrir código QR"><QrCodeIcon class="size-5" aria-hidden="true" /></a></div></li></ul>
      </template>
