  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="presentation" @click.self="emit('close')">
    <section class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-surface-raised shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="corrective-work-title">
      ...
      <form method="post" enctype="multipart/form-data" :action="data.routes.registerCorrective" class="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
        ...
        <fieldset class="grid gap-3 rounded-xl border border-border bg-surface-subtle p-4 sm:col-span-2 sm:grid-cols-4"><legend class="text-sm font-bold text-ink sm:col-span-4">Repuesto o insumo colocado (opcional)</legend><FormField label="Descripción" for-id="quick-corrective-part-description" class="sm:col-span-2"><input id="quick-corrective-part-description" name="repuestos[0][descripcion]" maxlength="255" placeholder="Ej.: Filtro de aceite" :class="fieldClass" /></FormField><FormField label="Cantidad" for-id="quick-corrective-part-quantity"><input id="quick-corrective-part-quantity" name="repuestos[0][cantidad]" type="number" min="0.001" step="0.001" value="1" :class="fieldClass" /></FormField><FormField label="Precio unitario" for-id="quick-corrective-part-price"><input id="quick-corrective-part-price" name="repuestos[0][precio_unitario]" type="number" min="0" step="0.01" value="0" :class="fieldClass" /></FormField><FormField label="Garantía hasta" for-id="quick-corrective-part-warranty" class="sm:col-span-2"><input id="quick-corrective-part-warranty" name="repuestos[0][garantia_fecha]" type="date" :class="fieldClass" /></FormField><p class="text-xs text-ink-muted sm:col-span-2">Para varios repuestos, completalos luego desde el detalle de la orden.</p></fieldset>
