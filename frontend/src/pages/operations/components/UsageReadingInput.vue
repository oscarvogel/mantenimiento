...
<template>
  <div class="grid gap-3 sm:grid-cols-2">
    <FormField v-if="equipment.controlsKm" label="Kilometraje total actual" :for-id="`${idPrefix}-km`">
      <span class="mb-1 block text-xs font-normal text-ink-muted">Último registrado: {{ formatKilometers(modelValue.currentKm) }}</span>
      <input :id="`${idPrefix}-km`" :name="names.kilometers" type="text" inputmode="numeric" autocomplete="off" :value="modelValue.kilometers" placeholder="Ingresá el total del equipo" :disabled="csrfDisabled" :class="fieldClass" @input="update('kilometers', $event.target.value)" />
