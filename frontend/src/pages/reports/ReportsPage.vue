...
import { ArrowDownTrayIcon, ChartBarSquareIcon } from '@heroicons/vue/24/outline'
...
const statusItems = computed(() => props.data.statusDistribution.map((item) => ({
  ...item,
...
<template>
  <div>
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div>
        <p class="flex items-center gap-2 text-sm font-semibold text-primary">
          <ChartBarSquareIcon class="size-5" aria-hidden="true" />
          Análisis operativo
        </p>
        ...
        <p class="mt-2 max-w-3xl text-sm leading-6 text-ink-muted">
          Costos, actividad y calidad de datos dentro de las sucursales que tenés autorizadas.
        </p>
      </div>
      <a :href="urls.export" class="inline-flex min-h-11 items-center justify-center gap-2 self-start rounded-lg border border-border-strong bg-surface-raised px-4 py-2.5 text-sm font-semibold text-ink shadow-sm hover:bg-surface-muted sm:self-auto">
    ...
    <form :action="urls.index" method="get" class="mt-7 grid gap-4 rounded-xl border border-border bg-surface-raised p-4 shadow-card sm:grid-cols-2 xl:grid-cols-[minmax(12rem,1fr)_repeat(3,minmax(9rem,0.7fr))_auto] xl:items-end">
      <label class="grid gap-1.5 text-sm font-semibold text-ink">
        ...
        <select name="sucursal_id" class="min-h-11 rounded-lg border border-border-strong bg-white px-3 text-sm font-normal text-ink focus:border-border-focus">
          ...
          <option v-for="branch in data.branches" :key="branch.id" :value="branch.id" :selected="Number(data.filters.branchId) === Number(branch.id)">
            {{ branch.nombre }}
          </option>
        </select>
      </label>
      <label class="grid gap-1.5 text-sm font-semibold text-ink">
        Desde
        <input type="date" name="desde" :value="data.filters.from" required class="min-h-11 rounded-lg border border-border-strong bg-white px-3 text-sm font-normal text-ink focus:border-border-focus" />
      </label>
      <label class="grid gap-1.5 text-sm font-semibold text-ink">
        Hasta
    ...
    <section aria-labelledby="report-summary" class="mt-6">
      <h2 id="report-summary" class="sr-only">Indicadores del período</h2>
    ...
    <div class="mt-6 grid gap-6 xl:grid-cols-2">
      <section aria-labelledby="status-title" class="rounded-xl border border-border bg-surface-raised p-5 shadow-card sm:p-6">
        <h2 id="status-title" class="text-lg font-bold text-ink">Órdenes por estado</h2>
        <p class="mb-5 mt-1 text-sm text-ink-muted">Órdenes cuya apertura ocurrió dentro del período seleccionado.</p>
        <SemanticBarList :items="statusItems" value-key="count" label-key="label" :value-formatter="number.format" />
      </section>
      <section aria-labelledby="cost-title" class="rounded-xl border border-border bg-surface-raised p-5 shadow-card sm:p-6">
        <h2 id="cost-title" class="text-lg font-bold text-ink">Costos por equipo</h2>
        <p class="mb-5 mt-1 text-sm text-ink-muted">Hasta 10 equipos con OT finalizadas.</p>
        <SemanticBarList :items="costItems" value-key="numericCost" label-key="equipmentCode" :value-formatter="money" />
      </section>
      <section aria-labelledby="evolution-title" class="rounded-xl border border-border bg-surface-raised p-5 shadow-card sm:p-6 xl:col-span-2">
        <h2 id="evolution-title" class="text-lg font-bold text-ink">Evolución de costos</h2>
        <p class="mb-5 mt-1 text-sm text-ink-muted">Agrupación {{ data.evolutionGranularity === 'day' ? 'diaria' : 'mensual' }} según la fecha de finalización.</p>
        <SemanticBarList :items="evolutionItems" value-key="numericCost" label-key="label" :value-formatter="money" />
      </section>
    </div>

    <section aria-labelledby="quality-title" class="mt-6 rounded-xl border border-border bg-surface-raised p-5 shadow-card sm:p-6">
      <h2 id="quality-title" class="text-lg font-bold text-ink">Calidad y disponibilidad de datos</h2>
      ...
      <ul class="mt-4 list-disc space-y-1 pl-5 text-sm leading-6 text-ink-muted">
        <li v-for="limitation in data.quality.limitations" :key="limitation">{{ limitation }}</li>
      </ul>
    </section>

    <section aria-labelledby="orders-title" class="mt-6 overflow-hidden rounded-xl border border-border bg-surface-raised shadow-card">
      <div class="border-b border-border px-5 py-4 sm:px-6">
        <h2 id="orders-title" class="text-lg font-bold text-ink">Órdenes del período</h2>
        <p class="mt-1 text-sm text-ink-muted">{{ data.orders.pagination.total }} resultados; listado paginado en servidor.</p>
      </div>
      <div v-if="data.orders.items.length" class="overflow-x-auto">
        <table class="min-w-full divide-y divide-border text-left text-sm">
          <thead class="bg-surface-muted text-xs font-semibold uppercase tracking-wide text-ink-muted">
            <tr><th class="px-5 py-3">Orden</th><th class="px-5 py-3">Equipo</th><th class="px-5 py-3">Sucursal</th><th class="px-5 py-3">Apertura</th><th class="px-5 py-3">Estado</th><th class="px-5 py-3 text-right">Costo</th></tr>
          </thead>
          <tbody class="divide-y divide-border-subtle">
            <tr v-for="order in data.orders.items" :key="order.id" class="hover:bg-surface-subtle">
              <td class="whitespace-nowrap px-5 py-4 font-semibold text-ink">{{ order.number }}</td>
              <td class="whitespace-nowrap px-5 py-4 text-ink">{{ order.equipmentCode }}</td>
              <td class="whitespace-nowrap px-5 py-4 text-ink-muted">{{ order.branchName }}</td>
              <td class="whitespace-nowrap px-5 py-4 text-ink-muted">{{ dateLabel(order.openedAt) }}</td>
              <td class="whitespace-nowrap px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-semibold" :class="statusTone(order.status)">{{ statusLabel(order.status) }}</span></td>
              <td class="whitespace-nowrap px-5 py-4 text-right font-semibold tabular-nums text-ink">{{ money(order.cost) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <nav v-if="pagination.totalPages > 1" aria-label="Paginación de órdenes" class="flex items-center justify-between border-t border-border px-5 py-4 text-sm">
        <a v-if="pagination.page > 1" :href="queryUrl(pagination.page - 1)" class="font-semibold text-primary hover:text-primary-hover">Anterior</a><span v-else></span>
        <span class="text-ink-muted">Página {{ pagination.page }} de {{ pagination.totalPages }}</span>
        <a v-if="pagination.page < pagination.totalPages" :href="queryUrl(pagination.page + 1)" class="font-semibold text-primary hover:text-primary-hover">Siguiente</a><span v-else></span>
      </nav>
    </section>
  </div>
</template>
