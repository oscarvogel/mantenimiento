...
Ambiente objetivo: staging interno `fasa_195`, puerto `8090`.
...
5. Ejecutar `php spark notifications:dispatch`.
6. Verificar que la notificación aparece una sola vez en el centro interno.
7. Ejecutar nuevamente el mismo collector/dispatch con la misma clave lógica y confirmar que no duplica la notificación ni la entrega.
...
1. volver al commit estable anterior de la aplicación;
2. no borrar tablas de notificaciones ni entregas si ya contienen auditoría real;
3. detener temporalmente el cron de `notifications:dispatch` durante la recuperación;
